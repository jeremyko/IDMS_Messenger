//*************************************************************************
// CMyButtonMenu.cpp : implementation file
// Version : 3.02
// Date : March 2002
// Author : Brent Corkum
// Email :  corkum@rocscience.com
// Latest Version : http://www.rocscience.com/~corkum/CMyButtonMenu.html
// 
...
#include "MyButtonMenu.h"        // CMyButtonMenu class declaration
...
UINT CMyButtonMenu::original_drawmode=CMyButtonMenu_DRAWMODE_ORIGINAL;
BOOL CMyButtonMenu::original_select_disabled=TRUE;
// how the menu's are drawn in winXP
UINT CMyButtonMenu::xp_drawmode=CMyButtonMenu_DRAWMODE_XP;
BOOL CMyButtonMenu::xp_select_disabled=FALSE;
BOOL CMyButtonMenu::xp_draw_3D_bitmaps=TRUE;

CImageList CMyButtonMenu::m_AllImages;
CArray<int,int&> CMyButtonMenu::m_AllImagesID;
...
CString CMyButtonMenuData::GetString(void)//returns the menu text in ANSI or UNICODE
...
CTypedPtrArray<CPtrArray, HMENU> CMyButtonMenu::m_AllSubMenus;  // Stores list of all sub-menus

IMPLEMENT_DYNAMIC( CMyButtonMenu, CMenu )
...
BOOL CMyButtonMenu::IsNewShell ()
{
	return (g_Shell>=Win95);
}

BOOL CMyButtonMenu::IsWinXPLuna()
{
	if(g_Shell==WinXP){
...
BOOL CMyButtonMenu::IsLunaMenuStyle()
{
	if(IsWinXPLuna()){
		if(xp_drawmode==CMyButtonMenu_DRAWMODE_XP)return(TRUE);
	}
	else{
		if(original_drawmode==CMyButtonMenu_DRAWMODE_XP)return(TRUE);
	}
	return(FALSE);
}

CMyButtonMenuData::~CMyButtonMenuData()
{
	if(bitmap)
		delete(bitmap);
...
BOOL CMyButtonMenu::IsMenu(CMenu *submenu)
{
	int m;
...
BOOL CMyButtonMenu::IsMenu(HMENU submenu)
{
	int m;
	int numSubMenus = m_AllSubMenus.GetUpperBound();
	for(m=0;m<=numSubMenus;++m){
...
BOOL CMyButtonMenu::DestroyMenu()
{
	// Destroy Sub menus:
	int m,n;
	...
	for(m = numSubMenus; m >= 0; m--){
		...
		BOOL flag=ptr->IsKindOf(RUNTIME_CLASS( CMyButtonMenu ));
		if(flag)delete((CMyButtonMenu *)ptr);
	}
...
int CMyButtonMenu::GetMenuDrawMode(void)
{
	if(IsWinXPLuna())return(xp_drawmode);
	return(original_drawmode);
}

BOOL CMyButtonMenu::GetSelectDisableMode(void)
{
...
/*
==========================================================================
void CMyButtonMenu::DrawItem(LPDRAWITEMSTRUCT)
---------------------------------------
...
void CMyButtonMenu::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	...
	if(pDC->GetDeviceCaps(RASTERCAPS) & RC_PALETTE)DrawItem_Win9xNT2000(lpDIS);
		...
		if(IsWinXPLuna()){
			if(xp_drawmode==CMyButtonMenu_DRAWMODE_XP) DrawItem_WinXP(lpDIS);
			else DrawItem_Win9xNT2000(lpDIS);
		}
		else{
			if(original_drawmode==CMyButtonMenu_DRAWMODE_XP) DrawItem_WinXP(lpDIS);
			else DrawItem_Win9xNT2000(lpDIS);
		}	
	}
}

void CMyButtonMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
	CRect rect;
	UINT state = (((CMyButtonMenuData*)(lpDIS->itemData))->nFlags);
	...
	if(state & MF_SEPARATOR){
		...
		if(lpDIS->itemData != NULL){
			nIconNormal = (((CMyButtonMenuData*)(lpDIS->itemData))->menuIconNormal);
			xoffset = (((CMyButtonMenuData*)(lpDIS->itemData))->xoffset);
			global_offset = (((CMyButtonMenuData*)(lpDIS->itemData))->global_offset);
			bitmap = (((CMyButtonMenuData*)(lpDIS->itemData))->bitmap);
			strText = ((CMyButtonMenuData*) (lpDIS->itemData))->GetString();
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			if(checkflag||standardflag||selectedflag||disableflag||state&ODS_CHECKED)
				rect2.SetRect(rect.left+m_iconX+4+MyButtonMenu_GAP,rect.top,rect.right,rect.bottom);
		...
		rect.left = rect.left + m_iconX + 8 + MyButtonMenu_GAP; 
...
COLORREF CMyButtonMenu::LightenColor(COLORREF col,double factor)
{
...
COLORREF CMyButtonMenu::DarkenColor(COLORREF col,double factor)
{
	if(factor>0.0&&factor<=1.0){
		BYTE red,green,blue,lightred,lightgreen,lightblue;
		red = GetRValue(col);
...
void CMyButtonMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
#ifdef CMyButtonMenu_USE_MEMDC
	CMyButtonMenuMemDC *pMemDC=NULL;
#endif
	CRect rect,rect2;
	UINT state = (((CMyButtonMenuData*)(lpDIS->itemData))->nFlags);
	...
	int CMyButtonMenu_PAD=4;
	if(xp_draw_3D_bitmaps)CMyButtonMenu_PAD=7;
	int barwidth=m_iconX+CMyButtonMenu_PAD;
	
	// remove the selected bit if it's grayed out
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled){
		if(lpDIS->itemState & ODS_SELECTED)lpDIS->itemState=lpDIS->itemState & ~ODS_SELECTED;
#ifdef CMyButtonMenu_USE_MEMDC
		pMemDC=new CMyButtonMenuMemDC(pDC,&lpDIS->rcItem);
		pDC = pMemDC;
	...
	if(state & MF_SEPARATOR){
		...
		rect.top+=rect.Height()>>1;
		rect.left = rect2.right+CMyButtonMenu_PAD;
		pDC->DrawEdge(&rect,EDGE_ETCHED,BF_TOP);
...
BOOL CMyButtonMenu::GetBitmapFromImageList(CDC* pDC,CImageList *imglist,int nIndex,CBitmap &bmp)
{
...
/*
==========================================================================
void CMyButtonMenu::MeasureItem(LPMEASUREITEMSTRUCT)
---------------------------------------------
...
void CMyButtonMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	UINT state = (((CMyButtonMenuData*)(lpMIS->itemData))->nFlags);
	int CMyButtonMenu_PAD=4;
	if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps)CMyButtonMenu_PAD=7;
	if(state & MF_SEPARATOR){
		lpMIS->itemWidth = 0;
		int temp = GetSystemMetrics(SM_CYMENU)>>1;
		lpMIS->itemHeight = temp>(m_iconY+CMyButtonMenu_PAD)/2 ? temp : (m_iconY+CMyButtonMenu_PAD)/2;
	}
	else{
		CFont m_fontMenu;
		LOGFONT m_lf;
		...
		const wchar_t *lpstrText = ((CMyButtonMenuData*)(lpMIS->itemData))->GetWideString();//SK: we use const to prevent misuse
		...
		if(IsLunaMenuStyle())lpMIS->itemWidth = m_iconX+CMyButtonMenu_PAD+8+t.cx;
		else lpMIS->itemWidth = m_iconX + t.cx + m_iconX + MyButtonMenu_GAP;
		int temp = GetSystemMetrics(SM_CYMENU);
		lpMIS->itemHeight = temp>m_iconY+CMyButtonMenu_PAD ? temp : m_iconY+CMyButtonMenu_PAD;
		m_fontMenu.DeleteObject();
	}
}

void CMyButtonMenu::SetIconSize (int width, int height)
{
	m_iconX = width;
	m_iconY = height;
}

BOOL CMyButtonMenu::AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CMyButtonMenu::AppendODMenuW(wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	...
	CMyButtonMenuData *mdata = new CMyButtonMenuData;
...
BOOL CMyButtonMenu::InsertODMenuA(UINT nPosition,LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CMyButtonMenu::InsertODMenuW(UINT nPosition,wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	if(!(nFlags & MF_BYPOSITION)){
		int iPosition =0;
		CMyButtonMenu* pMenu = FindMenuOption(nPosition,iPosition);
		if(pMenu){
...
BOOL CMyButtonMenu::ModifyODMenuA(const char * lpstrText,UINT nID,int nIconNormal)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,nIconNormal);//SK: see MFC Tech Note 059
}

BOOL CMyButtonMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	int nLoc;
	CMyButtonMenuData *mdata;
	CArray<CMyButtonMenu*,CMyButtonMenu*>bcsubs;
	CArray<int,int&>bclocs;
	
	// Find the old CMyButtonMenuData structure:
	CMyButtonMenu *psubmenu = FindMenuOption(nID,nLoc);
	do{
		if(psubmenu && nLoc>=0)mdata = psubmenu->m_MenuList[nLoc];
		else{
			// Create a new CMyButtonMenuData structure:
			mdata = new CMyButtonMenuData;
			m_MenuList.Add(mdata);
		}
...
BOOL CMyButtonMenu::ModifyODMenuA(const char * lpstrText,UINT nID,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,il,xoffset);
}

BOOL CMyButtonMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CImageList *il,int xoffset)
{
	int nLoc;
	CMyButtonMenuData *mdata;
	CArray<CMyButtonMenu*,CMyButtonMenu*>bcsubs;
...
BOOL CMyButtonMenu::ModifyODMenuA(const char * lpstrText,UINT nID,CBitmap *bmp)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,bmp);
}

BOOL CMyButtonMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
...
BOOL CMyButtonMenu::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,fill,border,hatchstyle,pSize);
}

BOOL CMyButtonMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
...
BOOL CMyButtonMenu::ModifyODMenuA(const char *lpstrText,const char *OptionText,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CMyButtonMenu::ModifyODMenuW(wchar_t *lpstrText,wchar_t *OptionText,
                           int nIconNormal)
{
	CMyButtonMenuData *mdata;
...
CMyButtonMenuData *CMyButtonMenu::NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string)
{
	CMyButtonMenuData *mdata;
...
BOOL CMyButtonMenu::LoadToolbars(const UINT *arID,int n)
{
	ASSERT(arID);
...
BOOL CMyButtonMenu::LoadToolbar(UINT nToolBar)
{
	UINT nID,nStyle;
	BOOL returnflag=FALSE;
	CMyButtonMenuToolBar bar;
	int xoffset=-1,xset;
	int iconx,icony;
...
BOOL CMyButtonMenu::LoadFromToolBar(UINT nID,UINT nToolBar,int& xoffset)
{
	int xset,offset;
	UINT nStyle;
	BOOL returnflag=FALSE;
...
// O.S.
CMyButtonMenuData *CMyButtonMenu::FindMenuItem(UINT nID)
{
	CMyButtonMenuData *pData = NULL;
	int i;
	...
	if (!pData){
		int loc;
		CMyButtonMenu *pMenu = FindMenuOption(nID, loc);
		ASSERT(pMenu != this);
		if (loc >= 0){
...
CMyButtonMenu *CMyButtonMenu::FindAnotherMenuOption(int nId,int& nLoc,CArray<CMyButtonMenu*,CMyButtonMenu*>&bcsubs,
									  CArray<int,int&>&bclocs)
{
	int i,numsubs,j;
	CMyButtonMenu *psubmenu,*pgoodmenu;
	BOOL foundflag;
	
	for(i=0;i<(int)(GetMenuItemCount());++i){
#ifdef _CPPRTTI 
		psubmenu=dynamic_cast<CMyButtonMenu *>(GetSubMenu(i));
#else
		psubmenu=(CMyButtonMenu *)GetSubMenu(i);
#endif
		if(psubmenu){
...
CMyButtonMenu *CMyButtonMenu::FindMenuOption(int nId,int& nLoc)
{
	int i;
	CMyButtonMenu *psubmenu,*pgoodmenu;
...
CMyButtonMenuData *CMyButtonMenu::FindMenuOption(wchar_t *lpstrText)
{
	int i,j;
	CMyButtonMenu *psubmenu;
	CMyButtonMenuData *pmenulist;
...
BOOL CMyButtonMenu::LoadMenu(int nResource)
{
	return(CMyButtonMenu::LoadMenu(MAKEINTRESOURCE(nResource)));
};

BOOL CMyButtonMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	...
	CTypedPtrArray<CPtrArray, CMyButtonMenu*>  m_Stack;    // Popup menu stack
	...
	do{
		...
		if(dwFlags & MF_POPUP){
			...
			CMyButtonMenu* pSubMenu = new CMyButtonMenu;
...
int CMyButtonMenu::GetMenuStart(void)
{
...
void CMyButtonMenu::RemoveTopLevelOwnerDraw(void)
{
	CString str;
	int i=0,j=0;
...
void CMyButtonMenu::InsertSpaces(void)
{
...
void CMyButtonMenu::LoadCheckmarkBitmap(int unselect, int select)
{
...
//--------------------------------------------------------------------------
//[18.06.99 rj]
BOOL CMyButtonMenu::GetMenuText(UINT id, CString& string, UINT nFlags/*= MF_BYPOSITION*/)
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags){
		...
		int uiLoc;
		CMyButtonMenu* pMenu = FindMenuOption(id,uiLoc);
...
void CMyButtonMenu::DrawRadioDot(CDC *pDC,int x,int y,COLORREF color)
{
...
void CMyButtonMenu::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color)
{
	CPen m_penBack;
...
CMyButtonMenuData *CMyButtonMenu::FindMenuList(UINT nID)
{
...
void CMyButtonMenu::InitializeMenuList(int value)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i)
		m_MenuList[i]->syncflag=value;
}

void CMyButtonMenu::DeleteMenuList(void)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i){
...
void CMyButtonMenu::SynchronizeMenu(void)
{
	CTypedPtrArray<CPtrArray, CMyButtonMenuData*> temp;
	CMyButtonMenuData *mdata;
	CString string;
...
void CMyButtonMenu::UpdateMenu(CMenu *pmenu)
{
#ifdef _CPPRTTI 
	CMyButtonMenu *psubmenu = dynamic_cast<CMyButtonMenu *>(pmenu);
#else
	CMyButtonMenu *psubmenu = (CMyButtonMenu *)pmenu;
#endif
	if(psubmenu)psubmenu->SynchronizeMenu();
}

LRESULT CMyButtonMenu::FindKeyboardShortcut(UINT nChar, UINT nFlags,
                                     CMenu *pMenu)
{
#ifdef _CPPRTTI 
	CMyButtonMenu *pCMyButtonMenu = dynamic_cast<CMyButtonMenu *>(pMenu);
#else
	CMyButtonMenu *pCMyButtonMenu = (CMyButtonMenu *)pMenu;
#endif
	if(pCMyButtonMenu && nFlags&MF_POPUP){
		CString key(_T('&'),2);//SK: modified for Unicode correctness
		key.SetAt(1,(TCHAR)nChar);
		key.MakeLower();
		CString menutext;
		int menusize = (int)pCMyButtonMenu->GetMenuItemCount();
		if(menusize!=(pCMyButtonMenu->m_MenuList.GetUpperBound()+1))
			pCMyButtonMenu->SynchronizeMenu();
		for(int i=0;i<menusize;++i){
			if(pCMyButtonMenu->GetMenuText(i,menutext)){
				menutext.MakeLower();
...
void CMyButtonMenu::DitherBlt (HDC hdcDest, int nXDest, int nYDest, int nWidth, 
...
void CMyButtonMenu::GetFadedBitmap(CBitmap &bmp)
{
	CDC ddc;
	COLORREF bgcol,col;
	BITMAP BitMap;
...
void CMyButtonMenu::GetShadowBitmap(CBitmap &bmp)
{
	CDC ddc;
...
BOOL CMyButtonMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	BOOL bReturn=FALSE;
...
void CMyButtonMenu::SetBitmapBackground(COLORREF color)
{
	m_bitmapBackground=color;
	m_bitmapBackgroundFlag=TRUE;
}

void CMyButtonMenu::UnSetBitmapBackground(void)
{
	m_bitmapBackgroundFlag=FALSE;
}
...
void CMyButtonMenu::AddFromToolBar(CToolBar* pToolBar, int nResourceID)
{
	for (int i = 0; i < pToolBar->GetCount(); i++) {
		...
		int nLoc;
		CMyButtonMenuData* pData;
		CMyButtonMenu *pSubMenu = FindMenuOption(nID, nLoc);
		if (pSubMenu && nLoc >= 0)pData = pSubMenu->m_MenuList[nLoc];
		else {
			// Create a new CMyButtonMenuData structure
			pData = new CMyButtonMenuData;
...
BOOL CMyButtonMenu::Draw3DCheckmark(CDC *dc, const CRect& rc,
...
BOOL CMyButtonMenu::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout)
{
	CBrush brushin;
...
void CMyButtonMenu::DitherBlt2(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
void CMyButtonMenu::SetDisableOldStyle(void)
{
	disable_old_style=TRUE;
}

void CMyButtonMenu::UnSetDisableOldStyle(void)
{
	disable_old_style=FALSE;
}

BOOL CMyButtonMenu::GetDisableOldStyle(void)
{
	return(disable_old_style);
}


WORD CMyButtonMenu::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
HBITMAP CMyButtonMenu::LoadSysColorBitmap(int nResourceId)
{
	HINSTANCE hInst = 
...
BOOL CMyButtonMenu::RemoveMenu(UINT uiId,UINT nFlags)
{
	int i = 0;
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			CMyButtonMenu* pSubMenu = (CMyButtonMenu*) GetSubMenu(uiId);
			if(NULL==pSubMenu){
		...
		int iPosition =0;
		CMyButtonMenu* pMenu = FindMenuOption(uiId,iPosition);
...
BOOL CMyButtonMenu::DeleteMenu(UINT uiId,UINT nFlags)
{
	int i = 0;
	if(MF_BYPOSITION&nFlags){
		UINT uint = GetMenuState(uiId,MF_BYPOSITION);
...
BOOL CMyButtonMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CMyButtonMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CMyButtonMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CMyButtonMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CMyButtonMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CMyButtonMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
BOOL CMyButtonMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL CMyButtonMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL CMyButtonMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL CMyButtonMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL CMyButtonMenu::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL CMyButtonMenu::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CMyButtonMenu* CMyButtonMenu::AppendODPopupMenuW(wchar_t *lpstrText)
{
	CMyButtonMenu* pSubMenu = new CMyButtonMenu;
	pSubMenu->m_unselectcheck=m_unselectcheck;
	pSubMenu->m_selectcheck=m_selectcheck;
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
CMyButtonMenu* CMyButtonMenu::AppendODPopupMenuA(LPCSTR lpstrText)
{
	USES_CONVERSION;
	return AppendODPopupMenuW(A2W(lpstrText));
}

BOOL CMyButtonMenu::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
...
CMenu* CMyButtonMenu::GetSubMenu(int nPos)
{
	return CMenu::GetSubMenu (nPos);
}

CMenu* CMyButtonMenu::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
CMyButtonMenu* CMyButtonMenu::GetSubCMyButtonMenu(char* pText)
{
	USES_CONVERSION;
	return GetSubCMyButtonMenu(A2W(pText));
}

CMyButtonMenu* CMyButtonMenu::GetSubCMyButtonMenu(wchar_t* lpszSubMenuName)
{
	CMyButtonMenuData *mdata;
	...
	if(mdata){
		...
		BOOL flag=ptr->IsKindOf(RUNTIME_CLASS( CMyButtonMenu ));
		if(flag)return((CMyButtonMenu *)ptr);
	}
	return NULL;
}
...
int CMyButtonMenu::GetMenuPosition(char* pText)
{
	USES_CONVERSION;
	return GetMenuPosition(A2W(pText));
}

int CMyButtonMenu::GetMenuPosition(wchar_t* pText)
{
	int i,j;
	CMyButtonMenu* psubmenu;
	for(i=0;i<(int)(GetMenuItemCount());++i)
...
int CMyButtonMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int CMyButtonMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
	...
	if(sPos == CMyButtonMenu_HEAD)
	{
		ASSERT(nPos - 1 >= 0);
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMyButtonMenu_TAIL)
	{
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMyButtonMenu_BOTH)
	{
...
int CMyButtonMenu::DeleteMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return DeleteMenu(A2W(pText), sPos);
}

int CMyButtonMenu::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
	int nPos = GetMenuPosition(pText);
	if(nPos != -1)
	{
...
// Larry Antram
BOOL CMyButtonMenu::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
...
// courtesy of Warren Stevens
void CMyButtonMenu::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	CDC bmpdc;
	COLORREF m_newclrBack;
...
BOOL CMyButtonMenu::IsWindowsClassicTheme(void)
{
...
int CMyButtonMenu::GlobalImageListOffset(int nID)
{
...
BOOL CMyButtonMenu::CanDraw3DImageList(int offset)
{
	BOOL retflag=FALSE;
	int numcurrent=m_AllImagesID.GetSize();
...
int CMyButtonMenu::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	int loc = -1;
...
BOOL CMyButtonMenuToolBar::LoadToolBar(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	ASSERT(lpszResourceName != NULL);
...
BOOL CMyButtonMenuToolBar::LoadBitmap(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	ASSERT(lpszResourceName != NULL);
