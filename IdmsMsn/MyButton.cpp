...
CMyButton::CMyButton()
{
	...
	// No associated menu
#ifndef	BTNST_USE_CMyButtonMenu
	m_hMenu = NULL;
#endif
	...
	m_bDrawBackColor_KEH = FALSE;
	m_crBKColor_KEH = ::GetSysColor(COLOR_BTNFACE);
	m_brBackground_KEH.CreateSolidBrush(m_crBKColor_KEH);
} // End of CMyButton
...
*/
	m_brBackground_KEH.DeleteObject();
...
{
	LRESULT lResult;
	if (CMyButtonMenu::IsMenu(pMenu))
		lResult = CMyButtonMenu::FindKeyboardShortcut(nChar, nFlags, pMenu);
	else
...
void CMyButton::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	...
	{
		if (IsMenu((HMENU)lpMeasureItemStruct->itemID) && CMyButtonMenu::IsMenu((HMENU)lpMeasureItemStruct->itemID))
		{
...
void CMyButton::OnEnable(BOOL bEnable) 
{
	...
	if(this->m_hWnd == NULL || ::IsWindow(this->m_hWnd) == FALSE)
	{
		return;
	}
...
#ifdef	BTNST_USE_CMyButtonMenu
			CMyButtonMenu* psub = (CMyButtonMenu*)m_menuPopup.GetSubMenu(0);
...
void CMyButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bDrawTransparent)
		PaintBk(pDC);
	else if(m_bDrawBackColor_KEH)
		OnDrawBackColor(pDC, &itemRect);
	else
...
void CMyButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsDisabled && m_bShowDisabledBitmap)
	{
		...
		int nOffset = abs( (int) (m_csBitmaps[byIndex].dwHeight - abs(rpItem->bottom-rpItem->top)) );
...
void CMyButton::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	// (★) 김은혁 수정 >> 왜? 체크박스는 텍스트 출력을 달리하기 위해! -> EX> 장애 등급 체크박스
	if(m_bIsCheckBox)
	{
		rpCaption->top += 3;
		rpCaption->left += 5;
		pDC->DrawText(lpszText, -1, rpCaption, DT_LEFT);	// 정렬방식이 기존의 Center방식이 아니라 Left정렬 사용!
															// 원래 체크박스는 Center방식 아님!
	}
	else
	{
	...
	*/
	if (m_bIsDisabled)
	{
		int nFormat = DT_WORDBREAK | DT_CENTER;
		if(m_bIsCheckBox)	// 이하 동문 (★)
		{
			nFormat = DT_LEFT;
		}
		...
		pDC->DrawText(lpszText, -1, rpCaption, nFormat);
		rpCaption->OffsetRect(-1, -1);
...
DWORD CMyButton::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
	...
	m_nBitmapIn_KEH  = nBitmapIn;
	m_nBitmapOut_KEH = nBitmapOut;
...
DWORD CMyButton::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	...
	if (hBitmapIn)
	{
		m_csBitmaps[0].hBitmap = m_hBitmapIn_Old_KEH = hBitmapIn;
		...
		if (hBitmapOut)
		{
			m_csBitmaps[1].hBitmap = m_hBitmapOut_Old_KEH = hBitmapOut;
...
// This function associates a menu to the button.
// The menu will be displayed clicking the button.
// The menu will be handled by the CMyButtonMenu class.
//
// Parameters:
//		[IN]	nMenu
//				ID number of the menu resource.
...
//		Where:
//				[IN]	wParam
//						If support for CMyButtonMenu is enabled: a pointer to CMyButtonMenu
...
void  CMyButton::AutoSetting(UINT nType, CWnd* wndParent)
{
	m_cwBtnUtil_KEH.AutoSetting(nType, this, wndParent);
}



void  CMyButton::AutoSize(CWnd* wndParent, int nBitmapID, BOOL bEraseText)
{
	CBitmap bm;
	BITMAP	info;
	...
	rect.right = rect.left + info.bmWidth + 3;
	rect.bottom = rect.top + info.bmHeight + 3;
...
void CMyButton::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	{
		if(pParent->m_hWnd)
			pParent->PostMessage(WM_BTNDOWN, (WPARAM)this, (LPARAM)NULL); 
	}
...
BOOL CMyButton::EnableWindow(BOOL bEnable)
{
	...
	if(!m_bEnable_KEH)
	{
		HBITMAP hBitmapOut_Gray = CreateGrayscaleBitmap(m_csBitmaps[1].hBitmap, m_csBitmaps[1].dwWidth, m_csBitmaps[1].dwHeight, m_csBitmaps[1].crTransparent);
		HBITMAP hBitmapIn_Gray = CreateGrayscaleBitmap(m_csBitmaps[0].hBitmap, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight, m_csBitmaps[0].crTransparent);
		...
		m_csBitmaps[1].hBitmap = hBitmapOut_Gray;
		m_csBitmaps[0].hBitmap = hBitmapIn_Gray;
	}
	else
	{
		SetBitmaps(m_nBitmapIn_KEH, RGB(255, 0, 255), m_nBitmapOut_KEH, RGB(255, 0, 255));
	}
	RedrawWindow();
...
CMenu* CMyButton::GetSubMenuEx(int nPos)
{
	return (m_menuPopup.GetSubMenu(nPos));
}



void  CMyButton::SetBkColorEx(COLORREF cColor)
{
	if(m_bDrawBackColor_KEH == FALSE)
		return;

	m_crBKColor_KEH = cColor; 
...
DWORD CMyButton::OnDrawBackColor(CDC* pDC, CRect* pRect)
{
	if(m_bDrawBackColor_KEH == FALSE)
		return BTNST_OK;

	pDC->FillRect(pRect, &m_brBackground_KEH);

	return BTNST_OK;
}


void CMyButton::DrawBackColor(BOOL bRepaint)
{
	m_bDrawBackColor_KEH = TRUE;
