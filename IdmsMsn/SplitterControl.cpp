...
/*
void CSplitterControl::SavePosition() 
{
	CRect rcWnd;
	GetWindowRect(rcWnd);
...
void CSplitterControl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bIsPressed)
	{
		...
		//DrawLine(&dc, m_nX, m_nY);
		//DrawLine(&dc, point.x, point.y); //test
		
		//CPoint pt(m_nX, m_nY);
		CPoint pt(point); //test
		...
		if (pOwner && IsWindow(pOwner->m_hWnd))
		{
			...
			if (m_nType == SPS_VERTICAL)
				delta = point.x - m_nSavePos; //delta = m_nX - m_nSavePos;
			else
				delta = point.y - m_nSavePos;    //delta = m_nY - m_nSavePos;  //TEST
			
			TRACE("delta [%d] m_nX [%d] m_nY [%d] m_nSavePos [%d] \n", delta , m_nX, m_nY, m_nSavePos );
