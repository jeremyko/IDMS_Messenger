...
#ifndef CMyButtonMenu_H
#define CMyButtonMenu_H

#include <afxtempl.h>

// CMyButtonMenuData class. Fill this class structure to define a single menu item:
class CMyButtonMenuData
{
	wchar_t *m_szMenuText;
public:
	CMyButtonMenuData () {menuIconNormal=-1;xoffset=-1;bitmap=NULL;pContext=NULL;
...
typedef enum {CMyButtonMenu_DRAWMODE_ORIGINAL,CMyButtonMenu_DRAWMODE_XP} BC_MenuDrawMode;

// how seperators are handled when removing a menu (Tongzhe Cui)
typedef enum {CMyButtonMenu_NONE, CMyButtonMenu_HEAD, CMyButtonMenu_TAIL, CMyButtonMenu_BOTH} BC_Seperator;
...
class CMyButtonMenu : public CMenu
{
	DECLARE_DYNAMIC( CMyButtonMenu )
public:
	CMyButtonMenu(); 
	virtual ~CMyButtonMenu();
	...
	CMyButtonMenu* AppendODPopupMenuA(LPCSTR lpstrText);
	CMyButtonMenu* AppendODPopupMenuW(wchar_t *lpstrText);
	...
	int RemoveMenu(char* pText, BC_Seperator sPos=CMyButtonMenu_NONE);
	int RemoveMenu(wchar_t* pText, BC_Seperator sPos=CMyButtonMenu_NONE);
	int DeleteMenu(char* pText, BC_Seperator sPos=CMyButtonMenu_NONE);
	int DeleteMenu(wchar_t* pText, BC_Seperator sPos=CMyButtonMenu_NONE);
	...
	CMyButtonMenu* GetSubCMyButtonMenu(char* lpszSubMenuName);
	CMyButtonMenu* GetSubCMyButtonMenu(wchar_t* lpszSubMenuName);
	...
	static void SetMenuDrawMode(UINT mode){
		CMyButtonMenu::original_drawmode=mode;
		CMyButtonMenu::xp_drawmode=mode;
	};
	...
	static void SetSelectDisableMode(BOOL mode){
		CMyButtonMenu::original_select_disabled=mode;
		CMyButtonMenu::xp_select_disabled=mode;
	};
	static int CMyButtonMenu::GetMenuDrawMode(void);
	static BOOL CMyButtonMenu::GetSelectDisableMode(void);

	// how the bitmaps are drawn in XP Luna mode
	static void SetXPBitmap3D(BOOL val){
		CMyButtonMenu::xp_draw_3D_bitmaps=val;
	};
	static BOOL GetXPBitmap3D(void){return CMyButtonMenu::xp_draw_3D_bitmaps;}
...
protected:
	...
	CMyButtonMenu *FindMenuOption(int nId,int& nLoc);
	CMyButtonMenu *FindAnotherMenuOption(int nId,int& nLoc,CArray<CMyButtonMenu*,CMyButtonMenu*>&bcsubs,
								  CArray<int,int&>&bclocs);
	CMyButtonMenuData *FindMenuOption(wchar_t *lpstrText);
	void InsertSpaces(void);
	void DrawCheckMark(CDC *pDC,int x,int y,COLORREF color);
	void DrawRadioDot(CDC *pDC,int x,int y,COLORREF color);
	CMyButtonMenuData *NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string);
...
// Member Variables
protected:
	CTypedPtrArray<CPtrArray, CMyButtonMenuData*> m_MenuList;  // Stores list of menu items 
	// When loading an owner-drawn menu using a Resource, CMyButtonMenu must keep track of
	// the popup menu's that it creates. Warning, this list *MUST* be destroyed
	// last item first :)
	CTypedPtrArray<CPtrArray, HMENU>  m_SubMenus;  // Stores list of sub-menus 
	// Stores a list of all CMyButtonMenu's ever created 
...
class CMyButtonMenuToolBar : public CToolBar{
public:
	CMyButtonMenuToolBar() : CToolBar() {m_iconX=m_iconY=0;}
...
#define CMyButtonMenu_USE_MEMDC

#ifdef CMyButtonMenu_USE_MEMDC
//////////////////////////////////////////////////
// CMyButtonMenuMemDC - memory DC
//
// Author: Keith Rule
...
class CMyButtonMenuMemDC : public CDC
{
public:

    // constructor sets up the memory DC
    CMyButtonMenuMemDC(CDC* pDC,LPCRECT lpSrcRect) : CDC()
    {
        ASSERT(pDC != NULL);
    ...
    CMyButtonMenuMemDC* operator->() {return this;}
        
    // Allow usage as a pointer
    operator CMyButtonMenuMemDC*() {return this;}

private:
    CBitmap  m_bitmap;      // Offscreen bitmap
    CBitmap* m_pOldBitmap;  // bitmap originally found in CMyButtonMenuMemDC
