...
void CDbLoginDlg::OnBtnDbLogin() 
{
	...
	// ID/PASS 확인		 
	strncpy(szSql, "SELECT 1 FROM DUAL" , strlen("SELECT 1 FROM DUAL"));
				
	// Oracle		
	COracleWorkerMT * pSqlWork = CMemoryPool::New_COracleWorkerMT();	//20060918
	pSqlWork->SetLoginInfo ( (LPCSTR)m_strConn, (LPCSTR)m_strUser, (LPCSTR)m_strPasswd);		
