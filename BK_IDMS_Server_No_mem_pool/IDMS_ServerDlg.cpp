...
CThreadSockAsyncServer *g_thread; //소켓작업용 Server 쓰레드
...
{	
	...
	SetTimer(300, CHK_TIMER_SEC , NULL);  // chk alive => 주기적으로 시간체크해서, 새벽 4시에 작업하게함
	...
	if( 300 == nIDEvent ) // chk alive
	{
		...
		*/
		...
		{			
			...
			{				
				// 서버기동 시간이 04 시 이전이면 당일 04 시로 설정.
				KillTimer(300);	
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 시간 설정[%s]" , (LPCSTR) m_strDailyChkAliveTime);	
			}
			else
			{
				// 그 이외는 다음날 04 시로 설정 
		...
		else 
		{
			if( ctTime.Format("%Y%m%d%H") == m_strDailyChkAliveTime )
			{
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 시작, 작업시간 [%s]" , (LPCSTR) m_strDailyChkAliveTime);	
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 종료, 새로운 작업시간 [%s]", (LPCSTR) m_strDailyChkAliveTime );	
