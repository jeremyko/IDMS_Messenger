...
//		}
		...
		BOOL process() throw() 
		{
			...
			{
				CLog::Instance().WriteFile(FALSE, "ServerSQLError",  "osess.Open() Error!! ErrNum [%d][%s] [%s]\n", 
					osess.ErrorNumber() ,osess.GetErrorText(), m_szSql);					 
			}

			if ( ! osess.IsOpen() )
			{		
				memset(m_szErrMsg ,0x00, sizeof(m_szErrMsg) );
				_snprintf(m_szErrMsg ,sizeof(m_szErrMsg)-1, "[Ln:%d] [%d] %s ", 
					__LINE__, osess.ServerErrorNumber(), osess.GetServerErrorText() );
			...
			else
			{
				...
				if( OFAILURE  == osess.BeginTransaction() )
				{
					...
					memset(m_szErrMsg ,0x00, sizeof(m_szErrMsg) );
					_snprintf(m_szErrMsg ,sizeof(m_szErrMsg)-1, "[Ln:%d] BeginTransaction Error!", __LINE__  );
			...
			if( OSUCCESS != nRslt)
			{
				CLog::Instance().WriteFile(FALSE, "ServerSQLError",  "Error!! ErrNum [%d][%s][%s] [%s]\n", 
					odb.ErrorNumber() , odb.GetErrorText(), osess.GetErrorText() , m_szSql);					 
			}

			if ( ! odb.IsOpen() )
			{ 
				memset(m_szErrMsg ,0x00, sizeof(m_szErrMsg) );
				_snprintf(m_szErrMsg ,sizeof(m_szErrMsg)-1, "[Ln:%d] %s [%d]", __LINE__, osess.GetErrorText(), m_nindex );
			...
			if( SQL_SELECT == m_nSqlMode )
			{
				...
				{
					memset(m_szErrMsg ,0x00, sizeof(m_szErrMsg) );
					_snprintf(m_szErrMsg ,sizeof(m_szErrMsg)-1, "[Ln: %d] [%d] %s [%d]", 
						__LINE__, odb.ServerErrorNumber(), odb.GetServerErrorText(), m_nindex );
							...
							case OTYPE_UINT:
								{
									...
									{
										memset(szTmpNum ,0x00, sizeof(szTmpNum) );
										_snprintf(szTmpNum ,sizeof(szTmpNum)-1, "%d", nlTmp );
							...
							case OTYPE_FLOAT:
								{
									...
									{										
										memset(szTmpNum ,0x00, sizeof(szTmpNum) );
										_snprintf(szTmpNum ,sizeof(szTmpNum)-1, "%f", nlTmp );
