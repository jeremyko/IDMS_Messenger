		...
		*/
		...
		{			
			...
			{				
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 시간 설정[%s]\n" , (LPCSTR) m_strDailyChkAliveTime);	
			}
			else
			{
				// 그 이외는 다음날 04 시로 설정 
		...
		else 
		{
			if( ctTime.Format("%Y%m%d%H") == m_strDailyChkAliveTime )
			{
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 시작, 작업시간 [%s]\n" , (LPCSTR) m_strDailyChkAliveTime);	
				...
				CLog::Instance().WriteFile(FALSE, "ChkSockAlive",  "ChkSockAlive 호출 종료, 새로운 작업시간 [%s]\n", (LPCSTR) m_strDailyChkAliveTime );	
