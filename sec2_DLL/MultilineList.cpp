   ...
   for (int row = 0; row < m_nRows; row++)
   {
      ...
      if (iRow == m_rowHeights.end())
      {
         ...
         for (int col = 0; col < m_nCols; col++)
         {
            ...
            // compute cell height
            m_offscreenDC.DrawText(cell.m_text,textRect,
...
void CMultilineList::RenderContent(CDC & dc, CRect & r)
{
	...
	for (; (row < m_nRows) && (y1 <= r.bottom); row++)
	{
		...
		for ( col = 0, x1 = r.left-m_viewXPos; (col < m_nCols); col++)
		{
			...
			dc.DrawText(cell.m_text,textRect,  DT_CALCRECT|DT_LEFT|DT_NOPREFIX|DT_TOP|DT_WORDBREAK);
			...
			else
			{
				...
				if( row == m_nColorRow1 || row == m_nColorRow2)
				{
					CRect bgRect(textRect);
					bgRect.InflateRect(INNER_PADDING,INNER_PADDING);
					bgRect.bottom = bgRect.top + m_rowHeights[row] + GRID_WIDTH;
					...
					hbrush = CreateSolidBrush(RGB(208, 207, 235));					
					hbrushOld = (HBRUSH)SelectObject(dc, hbrush);					

					// Do not forget to clean up.
					dc.FillRect(bgRect,CBrush::FromHandle(hbrush));
					dc.SetTextColor(RGB(0,0,200)); //2006.10.17
				...
				{
					dc.SetTextColor(RGB(111,122,111)); //2006.10.17
				}
			...
			dc.DrawText(cell.m_text,textRect, DT_LEFT|DT_NOPREFIX|DT_TOP|DT_WORDBREAK);
		...
		for (  col = 0, x1 = r.left-m_viewXPos; (col < m_nCols) && (x1 <= r.right); col++)
		{
			Column column;
			std::map<int,Column>::iterator i = m_columns.find(col);
			if (i != m_columns.end())
