...
/*
 SendDataToClient
	...
	1. ABCDEF, 123456789, LMOPQRST  동시에 전송
	2. ABCDEF 		EnqueueBufferObj(FALSE)	=> buf1
	3. 123456789	EnqueueBufferObj(FALSE)	=> buf2
	4. LMOPQRST		EnqueueBufferObj(FALSE)	=> buf3
	5. SendPendingData 에서 DequeueBufferObj(buf1), DequeueBufferObj(buf2), 
		=> send 시 buf1 은 성공 buf2 에서 4까지 보내고 WSAEWOULDBLOCK : sent1	
		=> context 전환..
    6. 다른 쓰레드가 이버퍼에 XXXX 저장 :buf4
	    => context 전환..

  이상태에서 버퍼는 
	buf3 -> buf4  
	
	7. 56789 을 위한 새로운 버퍼, EnqueueBufferObj(TRUE, Queue 앞에)	: buf5

  이상태에서 버퍼는 
	buf3(LMOPQRST) -> buf4(XXXX) -> buf5(56789)

  ABCDEF123456789LMOPQRST 보내야하는데 도중에 context 전환 되면서 
  
  ABCDEF1234LMOPQRSTXXXX56789 이런 데이터 받게된다. 처리가 안되는 경우 발생 
...
{	
	...
	if(mSockIdSockObjMap.Lookup( strSockID, ( CObject*& ) pa ))	// 20061204 	
	{
		if(pa )
		{
			...
			mSockIdSockObjMap.SetAt(strSockID, (CObject*) pa); // ip -> id 정보를 알수 있게 한다. (연결종료시 IP -> ID 구함) 20061204 
			...
			{ // ScopeLock 를 지역객체로 만든다. 
				...
				if(mIDIPMap.Lookup( (*itSet).c_str(), ( CObject*& ) pa ))
				{
					if( strcmp(pInfo->szIDFrom, (*itSet).c_str()) == 0 )
					{
						...
						//continue; // 이런식으로쓰면 위험하다.... LeaveCriticalSection 호출안되는경우발생 
						//else 로 분리하는게 안전 
					}
					else
					{
...
void CIocpServer::InformWhenDisconnected( CIocpNetObj * sock)
{
	...
	CLog::Instance().WriteFile(FALSE, "ServerLog", "Ln [%d] ☆ 연결이 끊어진 사용자 처리 Disconnected ip #2 [%s][%s]!\n", 
	...
	{ 			
		...
		CLog::Instance().WriteFile(FALSE, "ServerLog", "★ [%d] 연결이 끊어진 사용자 처리 RemoveSocketObj !\n", __LINE__);
	}
}
